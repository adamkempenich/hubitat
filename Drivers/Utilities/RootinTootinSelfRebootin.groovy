...
def stopIt(){
    log.info "Stoppin' the self-healing loops."
    unschedule()
}
...
def cancel(){
    unschedule()
    runIn(10, initialize)
    log.info "Cancelled the reboot loop. Scheduling the next reboot loop."
}

def reset(){
    log.error "Hub is unreachable. Rebooting now."
    httpPost([uri: "http://10.0.0.11:8080/hub/reboot"]) 
}

def initialize() {
    // Establish a connection to the device
    log.info "(Re)starting self-healing routine."
    unschedule() // remove any existing loops
    runIn(20, cancel) // if the DB write doesn't work, the next line won't be removed from the DB and will still fire (maybe)
    runIn(30, reset)
}
