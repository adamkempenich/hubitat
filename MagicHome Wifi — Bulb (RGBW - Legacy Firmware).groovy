...
metadata {
	definition (name: "MagicHome Wifi — Bulb (RGBW - Legacy Firmware)", namespace: "MagicHome", author: "Adam Kempenich") {
...
def setColor(parameters){
	...
	if( parameters.hue == 100 ){
		...
		data = [ 0x56, 0x00, 0x00, 0x00, device.currentValue( "level" ) * 2.55, 0x0f, 0xaa ]
	}
	else{
		// Update bulb's color
		...
		data =  [ 56, rgbColors.red, rgbColors.green, rgbColors.blue, 0x00, 0xf0, 0xaa ]
	}
